...
type UserFormData = IUser & FieldValues;
...
function LoginForm({ onToggleView }: { onToggleView: () => void }) {
  ...
  const onSubmit: SubmitHandler<UserFormData> = (data) => {
    ...
    if (!usuarioNoArquivo) {
      ...
      localStorage.setItem("nome", usuarioNoArquivo.nome);
    }
  ...
  return (
    <div className="w-full">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <button type="submit" className="w-full py-3 px-4 bg-gray-800 text-white font-semibold rounded-md hover:bg-gray-900 transition-colors">
          Entrar
        </button>
      </form>
...
export default function Acesso() {
  ...
  useEffect(() => {
    document.title = isAuthenticated ? "Minha Conta" : isLoginView ? "Login" : "Cadastro";
  }, [isLoginView, isAuthenticated]);
          ...
          <div className="space-y-6">
            {paciente && (
              <div className="bg-gray-50 p-4 rounded-lg">
                <h2 className="text-lg font-semibold text-gray-700 mb-3">Informações do Paciente</h2>
                <div className="space-y-2">
                  <p><span className="text-gray-600">Telefone:</span> {paciente.telefone}</p>
                  <p><span className="text-gray-600">Idade:</span> {paciente.idade} anos</p>
                  <div className="mt-2">
                    ...
                    <p className="text-gray-800">{paciente.endereco.cidade} - {paciente.endereco.cep}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <div className="space-y-4">
              <Link 
                to="/teleconsulta" 
                className="w-full block text-center py-3 px-4 bg-[var(--cor-destaque)] text-white font-semibold rounded-md hover:bg-[var(--cor-destaque-hover)] transition-colors"
              >
                Minhas Consultas
              </Link>
